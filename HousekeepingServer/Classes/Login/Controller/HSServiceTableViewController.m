...
- (void)viewDidLoad {
  [super viewDidLoad];
  self.title = @"服务类型选择";
     self.serviceSection = [[RETableViewSection alloc]initWithHeaderTitle:@"服务大类"];
...
- (void)loadService {
  ...
  [manager POST:serviceUrlStr
          ...
          _hud.completionBlock = ^{
            ...
            for (int i = 0; i < weakSelf.service.count; i++) {
              HSService *service = weakSelf.service[i];
                ...
                [weakSelf.serviceSection addItem:[RETableViewItem itemWithTitle:service.typeName accessoryType:UITableViewCellAccessoryDisclosureIndicator selectionHandler:^(RETableViewItem *item) {
      ...
      failure:^(AFHTTPRequestOperation *_Nonnull operation,
        ...
        [_hud hide:YES afterDelay:1.0];
        _hud.completionBlock = ^{
          weakSelf.serviceRefreshButton.frame = CGRectMake(XBScreenWidth * 0.5 - 50, XBScreenHeight * 0.3, 100, 30);
          ...
          success:^(AFHTTPRequestOperation *_Nonnull operation,
              ...
              if ([kServiceResponse isEqualToString:@"Success"]) {
                  ...
                  if (self.subService.count) {
                      ...
                      RETableViewOptionsController *subserviceOptionsVc = [[RETableViewOptionsController alloc]initWithItem:self.item options:self.subService multipleChoice:YES completionHandler:^(RETableViewItem *item) {
                          self.selectedString = @"";
                          for (int i = 0; i < self.item.value.count; i++) {
                              if (i != self.item.value.count - 1) {
                                  self.selectedString = [self.selectedString stringByAppendingString:self.item.value[i]];
                                  self.selectedString = [self.selectedString stringByAppendingString:@"|"];
                              }else{
                                  self.selectedString = [self.selectedString stringByAppendingString:self.item.value[i]];
                              }
                      ...
                      subserviceOptionsVc.title = [NSString stringWithFormat:@"当前已选-%@", parent];
                      HSInfoFooterView *footerView = [HSInfoFooterView footerView];
                      UIView *blankView =
                      [[UIView alloc] initWithFrame:CGRectMake(0, 0, XBScreenWidth, 90)];
                      HSOrangeButton *doneSelectBtn = [HSOrangeButton orangeButtonWithTitle:@"确认"];
                      CGFloat buttonX = 10;
                      ...
                      [weakSelf.navigationController pushViewController:subserviceOptionsVc animated:YES];
                  } else { // 没有子类
...
- (void)doneSelectBtnClicked{
    ...
    if (![self.selectedString isEqualToString:@""] && self.selectedString != nil) {
